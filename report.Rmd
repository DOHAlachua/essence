...
opts_chunk$set(comment = NA, message = FALSE, echo = FALSE, results = 'hide', fig.height = 3, warning = FALSE)
```
...
temp <- xtable(sub_voi)
print(temp, size = '\\tiny', include.rownames = FALSE)
```
...
plot2 <- 
  ggplot(data= df[df$Date== yesterday,], aes (x=Sex))+
  geom_bar(fill= "blue", aplha = 0.6) +
  theme_fivethirtyeight()+
  ggtitle("Yesterday")
...
Authorship, code, privacy, contact, etc...